...
def worker_process(zip_file, charset, start_idx, end_idx, file_name, result_queue, stop_event):
    with zipfile.ZipFile(zip_file, 'r') as zip_ref:
        ...
        for idx in range(start_idx, end_idx):
            ...
            if current_time - last_report >= 10:
                ...
                print(f"프로세스 {os.getpid()}: {progress:.1f}% | "
...
def unlock_zip(zip_file):
    ...
    print(f"테스트 파일: {smallest_info.filename} ({smallest_info.file_size} bytes)")
        ...
        try:
            ...
            for i in range(num_processes):
                ...
                task = pool.apply_async(
                    worker_process,
                    (zip_file, charset, start_idx, end_idx, smallest_info.filename, result_queue, stop_event)
                )
                tasks.append(task)
