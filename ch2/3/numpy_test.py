            ...
            with open(csv_files[i], 'r', encoding='utf-8') as f:
                ...
                if common_parts is None:
                    ...
                    if parts != common_parts:
                        raise ValueError(f"{csv_files[i]}의 parts 열이 다른 파일과 다릅니다!")
...
def filter_strength(header, parts, strengths):
    ...
    with open(outfile, 'w', newline='', encoding='utf-8') as out_csv:
        ...
        writer.writerows([part, strengths[i]] for i, part in enumerate(parts) if strengths[i] < 50)
...
def main():
    ...
    arr1, arr2, arr3 = [merged_strengths[0:len(merged_strengths), i].tolist() for i in range(merged_strengths.shape[1])]
